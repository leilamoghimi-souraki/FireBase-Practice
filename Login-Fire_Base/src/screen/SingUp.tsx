...
const App: FC = (props) => {
    ...
    const signUp = async () => {
        if (name && email && password) {
            try {
                const user = await initializeApp.auth().createUserWithEmailAndPassword(email, password);
                if (user) {
                    await initializeApp.firestore().collection('users').doc(user.uid).set({ name, email, password });
                }
            } catch (error) {
                console.log(error);
            }
